...
class ImportTransactionsService {
  async execute( filePath: string): Promise<Transaction[]> {
    ...
    const existentCategoriesTitles = existentCategories.map((category: Category) =>
     ...
     // o filter pode do filter para retirar duplicidades
    const addCategoryTitle = categories.filter( 
      category => !existentCategoriesTitles.includes(category)).filter((value, index, self) => self.indexOf(value) === index);
